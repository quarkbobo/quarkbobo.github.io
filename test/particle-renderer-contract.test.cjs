...
test('the generated home owns scene canvases while every themed page owns one cursor trail', () => {
  ...
  assert.equal(occurrences(home, 'id="planet-surface"'), 0)
  assert.equal(occurrences(home, 'id="planet-webgl"'), 1)
  ...
  assert.equal(occurrences(home, '<link rel="stylesheet" href="/css/planet-explorer.css">'), 1)
