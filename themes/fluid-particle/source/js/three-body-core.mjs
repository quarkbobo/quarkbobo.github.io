...
function validateParameters ({ G, mode, softening, boundary }) {
  range(G, 0.1, 5, '引力常数')
  range(softening, 0, 1, '软化长度')
  range(boundary, 6, 40, '人为边界')
...
export function updateBody (system, id, patch) {
  ...
  validateBody(next)
  for (const [key, limit] of [['position', 100], ['velocity', 20]]) {
    if (key in patch) next[key].forEach(value => range(value, -limit, limit, key))
  }
...
export function setParameters (system, patch) {
  if (!patch || Object.keys(patch).some(key => !['G', 'mode', 'softening', 'boundary'].includes(key))) throw new RangeError('未知系统参数')
  const next = { ...system, ...patch }
  validateParameters(next)
  if (('boundary' in patch || patch.mode === 1) && next.mode === 1 && next.bodies.some(body => body.active && body.position.some(value => Math.abs(value) + body.radius > next.boundary))) throw new RangeError('边界须容纳当前天体；请扩大边界或先调整位置。')
